...
uint16_t analogInput, tMode, stuID;
uint8_t TowerMajorVersion =1;
uint8_t TowerMinorVersion= 0;

// Function prototype
void FTM0Ch0Callback(const TFTMChannel* const aFTMChannel);
void FTM0Ch1Callback(const TFTMChannel* const aFTMChannel);
...
static BOOL AnalogInputValueCommand_50(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3);
...
TFTMChannel PacketTimer = {0,
			      ...
			      TIMER_OUTPUT_HIGH,
			      (void* )&FTM0Ch0Callback,
			      &PacketTimer};

static const TSPIModule aSPIModule =
{
  // TSPIModule Setup
  .isMaster                     = TRUE,       // Master
  .continuousClock              = FALSE,      // Continuous clock
  .inactiveHighClock            = FALSE,      // Inactive clock
  .changedOnLeadingClockEdge    = FALSE,      // Leading clock edge
  .LSBFirst                     = FALSE,      // MSB
...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
  ...
  /*FTMChannel0.channelNb=0;
    FTMChannel0.delayCount = CPU_MCGFF_CLK_HZ_CONFIG_0;//1 second
  ...
  FTMChannel1.userFunction = (void* )&FTM0Ch1Callback;
  ...
  // medRes=Median_Filter(testArray, 12);
  ...
  //Initialise the FTM Module
  FTM_Init();
  //Set the packet timer
  FTM_Set(&PacketTimer);
  //FTM_Set(&FTMChannel0);
  FTM_Set(&FTMChannel1);

  Analog_Init(CPU_BUS_CLK_HZ);
  // SPI_Init(&aSPIModule, CPU_BUS_CLK_HZ);

  ExitCritical();				// Exit critical component
  //FTM_StartTimer(&FTMChannel1);
...
static BOOL PacketHandler(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3)
{
  ...
  switch (command)
  {
    ...
    case 0x50:
    {
      //Set up interrupts on receipt of each packet with command 0x50
      //FTM_StartTimer(&FTMChannel0);
      FTM_StartTimer(&FTMChannel1);
      return AnalogInputValueCommand_50(command, parameter1, parameter2, parameter3);
      break;
    }
    case 0x0C: case 0x8C:
...
static BOOL StartupCommand_4(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3)
{
  if (command == 0x04)
  {
	...
	&& Packet_Put(0x50,1,0xFF & analogInput, analogInput >> 8));
    }
...
/*! @brief Provides instructions for handling analog input value command 0x50 coming from PC to tower
 * @param command is oldest char in the RxFIFO
 * @param parameter1 is 2nd oldest char in the RxFIFO
 * @param parameter2 is 3rd oldest char in the RxFIFO
 * @param parameter3 is 4th oldest char in the RxFIFO
...
static BOOL AnalogInputValueCommand_50(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3)
{
  ...
  switch(parameter1)
  {
    case 1:									// Get tower mode
      successExec=Packet_Put(0x7F&command, parameter1, 0xFF & analogInput, analogInput>>8);
      break;
    case 2:									// Set tower mode
      analogInput = (parameter3<<8) | parameter2;                            		// Noted parameter2 is LSB, parameter3 is MSB, value stored
      successExec = 1;
      break;
    default:
      invalidFlag = 1;
  ...
  if(command >> 7 == 1)								// Acknowledgment flag exists
  {
    ...
    return Packet_Put(command, parameter1, 0xFF & analogInput, analogInput >> 8);		// Leave ACK bit high to indicate success
  }
  else										// No acknowledgment required
  {
    if (invalidFlag = 1)
...
/*! @brief User callback function for FTM1_ISR
 *  @param aFTMChannel Structure containing the parameters used for setting up the timer channel
 *  @note Called following FTM0 interrupt meaning FTM0 timer has counter down to zero
 */
void FTM0Ch1Callback(const TFTMChannel* const aFTMChannel)
{
 ...
 // Analog_Get(0);
 // test.l = *Analog_Input[0].putPtr;
 // Packet_Put(0x50, 0, test.s.Lo, test.s.Hi);
  Analog_Get(1);
  test.l = *Analog_Input[1].putPtr;
  Packet_Put(0x50, 1, test.s.Lo, test.s.Hi);
}
